...
## PDE-Consistent Numerical Methods for Quantitative Finance

**Quantitative Finance Foundation**: Monte Carlo simulation as a **numerical weak solution of pricing PDEs** (Feynman-Kac formula), ensuring mathematical consistency across stochastic and PDE-based pricing methods.
...
### Mathematical Foundation: PDE-Consistent Monte Carlo

**Key Insight**: Monte Carlo simulation is treated as a **numerical weak solution of the pricing PDE**, ensuring mathematical consistency.
...
Has solution:
```
V(S_t, t) = E^Q[e^(-r(T-t)) Payoff(S_T) | S_t]
```

**Monte Carlo approximates this expectation**, providing a weak solution of the PDE.

### Simulation Model

Uses Geometric Brownian Motion (GBM) under risk-neutral measure Q:
...
Where:
- S: Asset price
- r: Risk-free rate (under Q measure)
- σ: Volatility
- dW^Q: Brownian motion under risk-neutral measure Q

### Numerical Methods

| Method | Purpose | Convergence |
|--------|---------|-------------|
| **Euler–Maruyama** | SDE simulation | O(Δt) |
| **Antithetic Variates** | Variance reduction | 2x efficiency |
| **Control Variates** | Variance reduction | 5-10x efficiency |
| **Importance Sampling** | Rare event simulation | Adaptive |

**Control Variates**: Using Black-Scholes closed form as control variate:
```
Price_MC = E^Q[Payoff] + β(E^Q[BS_Price] - BS_Price_theoretical)
```
Where β minimizes variance.
